<template>
    <v-app light>
        ...
        <v-navigation-drawer v-model="showRightNavPanel" floating right clipped disable-route-watcher disable-resize-watcher app fixed width="300px">
            <v-card class="ma-3">
                <v-card-text>
        ...
        <v-app-bar dark class="primary" fixed app clipped-left clipped-right v-if="!loading">
            <v-app-bar-nav-icon v-if="loggedIn"
                                 @click.native.stop="showMainNav = !showMainNav"></v-app-bar-nav-icon>
            ...
            <v-app-bar-nav-icon @click.native.prevent="toggleRightNav" :disabled="!hasTransaction" v-if="loggedIn"></v-app-bar-nav-icon>
        </v-app-bar>
        <v-content>
